declare module 'pandoc-wasm' {
  ...
  type Filter = 'citeproc' | string | { type: 'json' | 'lua' | 'citeproc'; path: string };
  ...
  interface Options {
    ...
    'html-math-method'?: { method: 'mathjax' | 'mathml' | 'webtex' | 'katex' | 'gladtex' | 'plain'; url?: string };
    'dump-args'?: boolean;
    'ignore-args'?: boolean;
  }
  ...
  async function convert(options: Options, stdin: string, files: Record<string, string | Blob>): Promise<ConvertReturn>;
  function query(options: Options): unknown;
  async function pandoc(args_str: string, inData: string | Blob, resources: Resource[] = []);
}
